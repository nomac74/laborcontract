...
def generate_contract(current_year, hourly_wage, start_hour, end_hour, weekly_working_days, break_time, employer_name, employee_name, job_title, contract_start, contract_end):
    ...
    contract_template = f"""
                ...
                제5조 【근로계약의 해지】
                    ...
                    6. 제 1 ~ 5항에 해당되는 때는 즉시 해고나 통지 후 근로계약을 해지 할 수 있다. 
                    7. 근로자가 개인적 사정으로 계약을 해지하고자 할 경우는'고용주'에게 30일전에 사직서을 제출하여야 한다.
                ...
                제7조 【기타】 
                    ...
                    이상의 계약을 명확히 하기 위하여 본 계약서 2부를 작성, '고용주'과'근로자'이 각각 (서명 or 날인) 후 각 1부씩 보관한다.
                    다만, 전자문서로 계약서의 SNS 통지도 가능하며, 통지된 근로계약서는 1년간 전자 저장매체에 보관한다.
                    \n\n\n\n
                    
                    【계약시작일】 : {start_date.strftime('%Y년 %m월 %d일')}
                    【계약종료일】 : {end_date.strftime('%Y년 %m월 %d일')}
                    \n\n\n
...
def main():
    ...
    try:
        weekly_working_days = float(weekly_working_days_input)
        if not 1 <= weekly_working_days <= 7:
            print("주당 근무일수는 1~7 사이의 숫자여야 합니다. 다시 입력해주세요.")
            weekly_working_days = None
    except ValueError:
